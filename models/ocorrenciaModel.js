...
async function getAllOcorrencias() {
  const query = "SELECT * FROM ocorrencia;"; // Define a query SQL para obter todos os registros da tabela "Users"
  return await executeQuery(query); // Executa a query usando a função executeQuery
}

async function getOcorrenciaById(id) {
  const query = "SELECT * FROM ocorrencia WHERE Ocorrencia_id = @id;"; // Define a query SQL para obter todos os registros da tabela "Users"
  const params = [{ name: "id", type: TYPES.Int, value: id }]; // Define o parâmetro @id;
  const ocorrencia = await executeQuery(query, params); // Executa a query usando a função executeQuery
  return ocorrencia.length > 0 ? ocorrencia[0] : null; // Retorna o primeiro usuário se houver algum resultado, ou null se não houver
}
...
//   const query = `INSERT INTO OcorrenciasRaiz (Criador, Data_ocorrencia, Hora, Iniciativa, Aspecto, Urgencia, Tema, Responsavel, Descricao, Encaminhamento, Statuss) VALUES (@Criador, @Data_ocorrencia, @Hora, @Iniciativa, @Aspecto, @Urgencia, @Tema, @Responsavel, @Descricao, @Encaminhamento, @Status);`; // Query SQL para inserir um novo registro
//   const params = [
//     { name: "Criador", type: TYPES.Int, value: Criador },
//     { name: "Data_ocorrencia", type: TYPES.Date, value: Data_ocorrencia },
//     { name: "Hora", type: TYPES.NVarChar, value: Hora },
...
//   const query = `UPDATE OcorrenciasRaiz SET Criador = @Criador, Data_ocorrencia = @Data_ocorrencia, Hora = @Hora, Iniciativa = @Iniciativa, Aspecto = @Aspecto, Urgencia = @Urgencia, Tema = @Tema, Responsavel = @Responsavel, Descricao = @Descricao, Encaminhamento = @Encaminhamento, Statuss = @Status WHERE Ocorrencia_id = @id;`; // Query SQL para atualizar o registro
//   const params = [
//     { name: "id", type: TYPES.Int, value: id }, // Define o parâmetro @id
//     { name: "Criador", type: TYPES.Int, value: Criador }, // Define o parâmetro @name
//     { name: "Data_ocorrencia", type: TYPES.Date, value: Data_ocorrencia },
