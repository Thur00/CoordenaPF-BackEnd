...
async function getAlunoByRm(rm) {
  const query = "SELECT * FROM Ocorrencias WHERE RM_aluno = @rm;"; // Query SQL com um parâmetro para filtrar pelo ID
...
async function getAlunoByNome(nome) {
  ...
  const params = [{ name: "nome", type: TYPES.NVarChar, value: `%${nome}%` }];
...
async function getAlunoByData(datas) {
  ...
  const query =
    "SELECT * FROM Ocorrencias WHERE Data_ocorrencia BETWEEN @data_inicial AND @data_final";
  const params = [
