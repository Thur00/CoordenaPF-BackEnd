...
async function getAlunoByNome(nome_completo) {
    ...
    const params = [{ name: "nome_completo", type: TYPES.NVarChar, value: nome_completo }];
...
async function getAlunoByData(data_inical, data_final) {
    const query = "SELECT * FROM Aluno WHERE DATA BETWEEN '@data_inical' AND '@data_final'";
    const params = [{ name: "data_inicial", type: TYPES.Date, value: data_inical }, { name: "data_final", type: TYPES.Date, value: data_final }];
    const alunos = await executeQuery(query, params);
    return alunos.length > 0 ? alunos[0] : null;
}
